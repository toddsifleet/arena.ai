...
@pytest.mark.asyncio
async def test_create_room(reg: ConnectionManager):
...
@pytest.mark.asyncio
async def test_list_room_ids_empty(reg: ConnectionManager):
    assert await reg.list_room_ids() == []


@pytest.mark.asyncio
async def test_list_room_ids(reg: ConnectionManager):
...
@pytest.mark.asyncio
async def test_join_room(reg: ConnectionManager):
    room_id = await reg.create_room()
...
@pytest.mark.asyncio
async def test_join_room_not_found(reg: ConnectionManager):
    with pytest.raises(RoomNotFound):
        await reg.join_room("nonexistent-room-id-long-enough")


@pytest.mark.asyncio
async def test_join_room_full(reg: ConnectionManager):
    room_id = await reg.create_room()
    await reg.join_room(room_id)
    await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_join_room_reconnect_slot(reg: ConnectionManager):
    room_id = await reg.create_room()
...
@pytest.mark.asyncio
async def test_join_room_already_connected(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_reconnect_detection(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_unregister_sets_disconnected_at(reg: ConnectionManager):
...
@pytest.mark.asyncio
async def test_peer_in_room(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_get_other_peers_in_room(reg: ConnectionManager):
    room_id = await reg.create_room()
...
@pytest.mark.asyncio
async def test_remove_peer_from_room(reg: ConnectionManager):
    room_id = await reg.create_room()
    a = await reg.join_room(room_id)
    b = await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_remove_peer_destroys_room_when_last(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_remove_unknown_peer_noop(reg: ConnectionManager):
...
@pytest.mark.asyncio
async def test_list_peers_unknown_room(reg: ConnectionManager):
...
@pytest.mark.asyncio
async def test_get_stale_peer_ids(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
    await reg.register_peer_ws(result.peer_id, object())
...
@pytest.mark.asyncio
async def test_get_peers_past_reconnect_grace(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
    await reg.register_peer_ws(result.peer_id, object())
    await reg.unregister_peer_ws(result.peer_id)
...
@pytest.mark.asyncio
async def test_get_all_peer_connections(reg: ConnectionManager):
    room_id = await reg.create_room()
    a = await reg.join_room(room_id)
    b = await reg.join_room(room_id)
...
@pytest.mark.asyncio
async def test_presence_subs_add_get_remove(reg: ConnectionManager):
    room_id = await reg.create_room()
...
@pytest.mark.asyncio
async def test_snapshot_empty(reg: ConnectionManager):
...
@pytest.mark.asyncio
async def test_snapshot_with_connected_peer(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
    await reg.register_peer_ws(result.peer_id, object())
...
@pytest.mark.asyncio
async def test_snapshot_with_disconnected_peer(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
    await reg.register_peer_ws(result.peer_id, object())
    await reg.unregister_peer_ws(result.peer_id)
...
@pytest.mark.asyncio
async def test_shutdown(reg: ConnectionManager):
    room_id = await reg.create_room()
    result = await reg.join_room(room_id)
    await reg.register_peer_ws(result.peer_id, object())
